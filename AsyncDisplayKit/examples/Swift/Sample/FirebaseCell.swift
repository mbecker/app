...
class FirebaseCell: ASCellNode, ASCollectionDelegate, ASCollectionDataSource {
  ...
  init(section: Section, sectionSize: CGSize) {
    ...
    self.itemSize = CGSize(width: sectionSize.width - 40, height: (sectionSize.width - 40) / 1.61803398875 + 40)
  ...
  func viewFrame() -> CGRect {
    return CGRect(x: 0, y: 0, width: self.sectionSize.width, height: self.sectionSize.height + 20)
  }
  ...
  override func calculateLayoutThatFits(_ constrainedSize: ASSizeRange) -> ASLayout {
    ...
    return ASLayout(layoutElement: self, size: CGSize(width: self.sectionSize.width, height: self.sectionSize.height + 20))
  }
...
//      return nodeBlock
  ...
  func scrollViewWillEndDragging(_ scrollView: UIScrollView, withVelocity velocity: CGPoint, targetContentOffset: UnsafeMutablePointer<CGPoint>) {
    let pageWidth:CGFloat = scrollView.frame.width - 40 + 10
